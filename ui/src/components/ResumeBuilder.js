...
export default function ResumeBuilder() {
    ...
    const renderInput = (label, value, onChange, placeholder, type = "text", isTextArea = false) => (
        <div className="flex flex-col gap-1 w-full">
            <label className="text-xs text-[#5C5550] font-medium">{label}</label>
            {isTextArea ? (
                <textarea
                    value={value}
                    onChange={(e) => onChange(e.target.value)}
                    placeholder={placeholder}
                    className="w-full rounded-lg bg-[#1C1C1C] border border-[#2A2520] text-sm text-[#F5F0EB] px-3 py-2 outline-none resize-y min-h-[60px] focus:border-[#FF6B35]/50 transition-colors"
                />
            ) : (
                <input
                    type={type}
                    ...
                    onChange={(e) => onChange(e.target.value)}
                    placeholder={placeholder}
                    className="w-full rounded-lg bg-[#1C1C1C] border border-[#2A2520] text-sm text-[#F5F0EB] px-3 py-2 outline-none focus:border-[#FF6B35]/50 transition-colors"
                />
            )}
        </div>
    );
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                <h1 className="text-2xl font-bold font-['Syne'] text-[#F5F0EB] border-l-2 border-[#FF6B35] pl-3">Resume Builder</h1>
                <div className="flex gap-3 flex-wrap items-center">
                    <button onClick={handleAdd} disabled={loading} className="px-4 py-2 rounded-lg bg-gradient-to-r from-[#FF6B35] to-[#FF3D00] font-semibold hover:scale-105 hover:text-white transition-colors text-sm">Save Data</button>
                    ...
                    <button onClick={() => setShowPreview(true)} className="px-4 py-2 rounded-lg bg-gradient-to-r from-[#FF6B35] to-[#E8A430] text-sm font-semibold hover:opacity-90 transition-opacity flex items-center gap-2 text-white">
            ...
            {/* Template Picker */}
            <div className="rounded-xl bg-[#1C1C1C] border border-[#2A2520] p-5">
                <h2 className="font-semibold mb-3 font-['Syne'] text-[#F5F0EB]">Choose Template</h2>
                <div className="flex gap-4 overflow-x-auto pb-2 custom-scrollbar">
                    {TEMPLATES.map((t) => (
                        <button
                            ...
                            className={`shrink-0 w-32 h-44 rounded-xl border-2 transition-all flex flex-col overflow-hidden relative ${template === t.id ? 'border-[#FF6B35] shadow-[0_0_15px_rgba(255,107,53,0.5)] scale-105' : 'border-[#2A2520] opacity-60 hover:opacity-90 hover:border-[#FF6B35]/30'
                                }`}
                        >
                            {/* Fallback pattern if image is missing */}
                            <div className="w-full h-full bg-[#1C1C1C] absolute inset-0 flex items-center justify-center p-2 text-center text-xs text-[#5C5550]">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[220px_1fr] gap-4">
                {/* Section Tabs */}
                <div className="rounded-xl bg-[#1C1C1C] border border-[#2A2520] p-3 flex lg:flex-col gap-2 overflow-x-auto lg:overflow-visible h-fit">
                    {SECTIONS.map((s) => (
                        <button
                            key={s.id}
                            type="button"
                            onClick={() => setActiveField(s.id)}
                            className={`text-left px-3 py-2.5 rounded-lg text-sm whitespace-nowrap transition-colors ${activeField === s.id ? 'bg-[#FF6B35] text-white font-semibold shadow-md' : 'text-[#A89E94] hover:text-white hover:bg-[#2A2520]'
                                }`}
                        >
                            {s.title}
                        </button>
                ...
                {/* Editing Area */}
                <div className="rounded-xl bg-[#1C1C1C] border border-[#2A2520] p-5 min-h-[400px]">

                    {/* PERSONAL INFO */}
                    {activeField === 'personalInfo' && (
                        <div className="space-y-4">
                            <h3 className="font-semibold text-lg font-['Syne'] text-[#F5F0EB] border-b border-[#2A2520] pb-2 mb-4">Personal Information</h3>
                            ...
                            {template === 'Template2' && (
                                <div className="mt-6">
                                    <label className="text-xs text-[#5C5550] font-medium block mb-2">Profile Picture (Required for this template)</label>
                                    <div className="flex items-center gap-4">
                                        <div className="w-20 h-20 rounded-lg bg-[#2A2520] overflow-hidden border border-[rgba(255,107,53,0.2)] flex items-center justify-center relative">
                                            {personalInfo.profileImage ? (
                                                <img src={personalInfo.profileImage} alt="Profile" className="w-full h-full object-cover" />
                                            ) : (
                                                <span className="text-[#5C5550] text-xs text-center px-2">No Image</span>
                                            )}
                                        </div>
                                        <label className="cursor-pointer px-4 py-2 bg-[#2A2520] hover:bg-[#FF6B35]/20 border border-[rgba(255,107,53,0.2)] rounded-lg text-sm text-white flex items-center gap-2 transition-colors">
                    ...
                    {activeField === 'summary' && (
                        <div className="h-full flex flex-col">
                            ...
                            <textarea
                                ...
                                className="w-full flex-1 min-h-[250px] rounded-lg bg-[#1C1C1C] border border-[#2A2520] text-sm text-[#F5F0EB] px-4 py-3 outline-none resize-none placeholder:text-[#5C5550] focus:border-[#FF6B35]/50 transition-colors leading-relaxed"
                            />
                        </div>
                    )}
                    ...
                    {activeField === 'experience' && (
                        <div>
                            <h3 className="font-semibold text-lg font-['Syne'] text-[#F5F0EB] border-b border-[#2A2520] pb-2 mb-4">Work Experience</h3>
                            <div className="space-y-6">
                                {experience.map((exp, idx) => (
                                    <div key={idx} className="relative p-4 rounded-xl border border-[#2A2520] bg-[#1C1C1C]">
                                ...
                                <button onClick={addExperience} className="flex items-center gap-2 px-4 py-2 text-sm font-medium text-[#FF6B35] hover:text-white bg-[#1C1C1C] hover:bg-[#FF6B35]/20 rounded-lg transition-colors border border-[#2A2520]">
                    ...
                    {activeField === 'education' && (
                        <div>
                            <h3 className="font-semibold text-lg font-['Syne'] text-[#F5F0EB] border-b border-[#2A2520] pb-2 mb-4">Education & Training</h3>
                            <div className="space-y-4">
                                ...
                                <button onClick={addEducation} className="flex items-center gap-2 px-4 py-2 text-sm font-medium text-[#FF6B35] hover:text-white bg-[#1C1C1C] hover:bg-[#FF6B35]/20 rounded-lg transition-colors border border-[#2A2520]">
                    ...
                    {activeField === 'languages' && (
                        <div>
                            <h3 className="font-semibold text-lg font-['Syne'] text-[#F5F0EB] border-b border-[#2A2520] pb-2 mb-4">Languages</h3>
                            <div className="space-y-4">
                                ...
                                <button onClick={addLanguage} className="flex items-center gap-2 px-4 py-2 mt-2 text-sm font-medium text-[#FF6B35] hover:text-white bg-[#1C1C1C] hover:bg-[#FF6B35]/20 rounded-lg transition-colors border border-[#2A2520] w-fit">
                    ...
                    {activeField === 'skills' && (
                        <div>
                            <h3 className="font-semibold text-lg font-['Syne'] text-[#F5F0EB] border-b border-[#2A2520] pb-2 mb-4">Skills</h3>
                            <div className="space-y-3 max-w-lg">
                                {skills.map((item, idx) => (
                                    <div key={idx} className="flex gap-2">
                                        <input
                                            ...
                                            className="flex-1 rounded-lg bg-[#1C1C1C] border border-[#2A2520] text-sm text-[#F5F0EB] px-4 py-2.5 outline-none focus:border-[#FF6B35]/50 transition-colors"
                                ...
                                <button onClick={() => addSimpleList(setSkills, skills)} className="flex items-center gap-2 px-4 py-2 mt-2 text-sm font-medium text-[#FF6B35] hover:text-white bg-[#1C1C1C] hover:bg-[#FF6B35]/20 rounded-lg transition-colors border border-[#2A2520] w-fit">
                    ...
                    {activeField === 'projects' && (
                        <div>
                            <h3 className="font-semibold text-lg font-['Syne'] text-[#F5F0EB] border-b border-[#2A2520] pb-2 mb-4">Projects</h3>
                            <div className="space-y-3">
                                {projects.map((item, idx) => (
                                    <div key={idx} className="flex gap-2 items-start">
                                        <textarea
                                            ...
                                            className="flex-1 rounded-lg bg-[#1C1C1C] border border-[#2A2520] text-sm text-[#F5F0EB] px-4 py-2.5 outline-none focus:border-[#FF6B35]/50 transition-colors min-h-[60px] resize-y placeholder:text-[#5C5550]"
                                ...
                                <button onClick={() => addSimpleList(setProjects, projects)} className="flex items-center gap-2 px-4 py-2 mt-2 text-sm font-medium text-[#FF6B35] hover:text-white bg-[#1C1C1C] hover:bg-[#FF6B35]/20 rounded-lg transition-colors border border-[#2A2520] w-fit">
                    ...
                    <div className="bg-[#f0f2f5] rounded-xl max-w-5xl w-full max-h-[90vh] flex flex-col overflow-hidden text-black shadow-2xl border border-gray-300">
                        ...
                        <div className="p-4 border-b border-gray-300 flex justify-between items-center bg-white flex-shrink-0 shadow-sm z-10">
                            <h3 className="font-bold text-lg text-gray-800 flex items-center gap-2">
                                <FileText size={20} className="text-[#FF6B35]" /> Resume Preview ({template})
                            </h3>
                            <div className="flex gap-2">
                                <button onClick={handleDownload} className="px-5 py-2 bg-gradient-to-r from-[#FF6B35] to-[#FF3D00] text-white rounded-md font-semibold text-sm hover:scale-105 transition-colors flex items-center gap-2 shadow-sm">
                                        ...
                                        <div className="flex mt-2 mb-8 items-center gap-8 border-b-2 border-gray-300 pb-8">
                                            ...
                                            <div className="w-[30%]">
                                                {personalInfo.profileImage ? (
                                                    ...
                                                    <div className="w-[120px] h-[120px] bg-gray-200 flex items-center justify-center text-[#5C5550] text-xs">Photo</div>
                                                )}
                                            </div>
                                            {/* Header Content Column */}
                                            <div className="w-[70%] text-[#5C5550]">
                                                    ...
                                                    <div className="space-y-5">
                                                        {experience.filter(e => e.role || e.company).map((exp, i) => (
                                                            <div key={i} className="text-[11px] text-black flex">
                                                                <div className="w-[20%] text-[#5C5550]">{exp.date}</div>
                                                    ...
                                                    <div className="space-y-3">
                                                        {education.filter(e => e.degree || e.institute).map((edu, i) => (
                                                            <div key={i} className="text-[11px] text-black flex font-bold">
                                                                <div className="w-[20%] text-[#5C5550] font-normal">{edu.year}</div>
