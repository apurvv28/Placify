...
export default function ChatSection() {
    ...
    const fetchMessages = async () => {
      try {
        ...
        if (res.ok) {
          ...
          await fetch(`http://localhost:5000/api/messages/seen/${selectedUser._id}`, {
            method: 'PATCH',
            headers: { Authorization: `Bearer ${token}` }
          });
  ...
  useEffect(() => {
    ...
    const handleNewMessage = (msg) => {
      ...
      if (isFromSelected) {
        ...
        fetch(`http://localhost:5000/api/messages/seen/${msg.senderId}`, {
          method: 'PATCH',
          headers: { Authorization: `Bearer ${token}` }
        });
      } else if (isForMe) {
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      const res = await fetch(`http://localhost:5000/api/messages/send/${selectedUser._id}`, {
        method: 'POST',
        headers: { 
  ...
  const handleToggleReaction = async (messageId, emoji) => {
    try {
      const res = await fetch(`http://localhost:5000/api/messages/react/${messageId}`, {
        method: 'PATCH',
        headers: { 
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}` 
  ...
  const handleClearChat = async () => {
    ...
    try {
      const res = await fetch(`http://localhost:5000/api/messages/clear/${selectedUser._id}`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${token}` }
      });
      if (res.ok) {
  ...
  const handleBlockUser = async () => {
    ...
    try {
      const res = await fetch(`http://localhost:5000/api/users/${action}/${selectedUser._id}`, {
        method: 'PATCH',
        headers: { Authorization: `Bearer ${token}` }
      });
      if (res.ok) {
